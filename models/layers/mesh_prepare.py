...
def slide_verts(mesh, prct):
    ...
    dihedral = dihedral_angle(mesh, edge_points, None).squeeze() #todo make fixed_division epsilon=0
...
def extract_features(mesh,opt):
    ...
    extractor_list = [dihedral_angle, symmetric_opposite_angles, symmetric_ratios]
    if opt.features is not None:
        if opt.features.find("dist") != -1:
            extractor_list.append(origo_dist)
        if opt.features.find("xyz") != -1:
            extractor_list.append(mid_point)
        if opt.features.find("curvature") != -1:
            extractor_list.append(mesh_curvature)
    
    
    with np.errstate(divide='raise'):
        try:
            for extractor in extractor_list:
                feature = extractor(mesh, edge_points, opt)
...
def dihedral_angle(mesh, edge_points, opt):
...
def symmetric_opposite_angles(mesh, edge_points, opt):
...
def symmetric_ratios(mesh, edge_points, opt):
...
def origo_dist(mesh, edge_points, opt):
    points = mid_point(mesh, edge_points, opt)
    o_dists = np.sqrt((points**2).sum(0))
    return np.expand_dims(o_dists, axis=0)

def mid_point(mesh, edge_points, opt):
    midpoint = np.zeros((mesh.edges_count,3))
    for edge_id, edge in enumerate(mesh.edges):
        midpoint[edge_id] = (mesh.vs[edge[0]] + mesh.vs[edge[1]]) / 2
        
    return midpoint.T

def mesh_curvature(mesh, edge_points, opt):
    """
    For online calculation of curvature
    
    reader = vtk.vtkPolyDataReader()
    reader.SetFileName(mesh.filename)
    reader.Update()
    ...
    num = ''.join([n for n in opt.features if n.isdigit()])
    if num:
        r = float(num[-1])/10**(len(num)-1)
    else:
        r = 0.08
            
    mid_points = mid_point(mesh, 0, opt).T
    edge_curvature = np.zeros(len(mid_points))
    for idx,point in enumerate(mid_points):
        l = vtk.vtkIdList()
        locator = vtk.vtkPointLocator()
        locator.SetDataSet(source)
        locator.BuildLocator()
        locator.FindPointsWithinRadius(r,point,l)
        
        neighbours = [l.GetId(i) for i in range(l.GetNumberOfIds())] 
        vals = [source.GetPointData().GetScalars(curvature).GetValue(i) for i in neighbours] 
        edge_curvature[idx] = np.asarray(vals).mean()
        if not vals:
            edge_curvature[idx] = 0
    """
    file_num = os.path.split(mesh.filename)[-1].split('.')[0]
    
    num = []
    for f in opt.features.split('_'):
        if f.find("curvature") != -1:
            num.append( ''.join([n for n in f if n.isdigit()]))
    edge_curvature = []
    for n in num:
        npz_name = opt.dataroot + '/features/curvature'+n + '/' + file_num + '.npz'
        
        loaded = np.load(npz_name)
        edge_curvature.append(np.clip(loaded['curvature'],-5, 5).T)
    edge_curvature = np.asarray(edge_curvature)
    return  edge_curvature #np.expand_dims(np.clip(edge_curvature,-5, 5), axis=0)
