...
def define_classifier(input_nc, ncf, ninput_edges, nclasses, opt, gpu_ids, arch, init_type, init_gain):
    ...
    elif arch == 'meshdistance':
        down_convs = [input_nc] + ncf
        up_convs = ncf[::-1] + [nclasses]
        pool_res = [ninput_edges] + opt.pool_res
        net = MeshDistanceField(pool_res, down_convs, up_convs, blocks=opt.resblocks,
                                 transfer_data=True, nl_block = opt.nl_block)
    else:
...
def define_loss(opt):
    if opt.dataset_mode == 'classification':
        ...
        loss = torch.nn.CrossEntropyLoss( ignore_index=-1)
    elif opt.dataset_mode == 'distance_field':
        loss = weighted_loss(opt)
    return loss
...
class weighted_loss(nn.Module):
    def __init__(self,opt):
        ...
        if opt.dataroot.split('/')[1].split('_')[-1][0:3] == 'inv':
            self.inv = True
            self.clamp = 0
        else:
            self.inv = False
    ...
    def normal_dist(self, x , mean , sd):
        prob_density = 1/(np.sqrt(2*np.pi)*sd) * torch.exp(-0.5*((x-mean)/sd)**2)
        return prob_density        
    def forward(self,output: torch.Tensor, target: torch.Tensor,):
        
        if self.inv:
            target =torch.clamp(target,min = self.clamp, max = 1)
            tmp = (target+0.001) *(target-output)**2
        elif self.gauss:
            target = self.normal_dist(target, 0 ,self.sd)
            target /= target.max()
            tmp = (target-output)**2
            if self.weight: tmp *= (target + 0.5)
        else:
            target =torch.clamp(target,min=0, max = self.clamp)
            tmp =  1/(target+0.001) *(target-output)**2
        loss = torch.mean(tmp)
        return loss
...
class MeshDistanceField(nn.Module):
    """Network for fully-convolutional tasks (Distance Field)
    """
    def __init__(self, pools, down_convs, up_convs, blocks=0, transfer_data=True, nl_block = 0):
        super(MeshDistanceField, self).__init__()
        self.transfer_data = transfer_data
        self.encoder = MeshEncoder(pools, down_convs, blocks=blocks, nl_block = nl_block)
        unrolls = pools[:-1].copy()
        unrolls.reverse()
        self.decoder = MeshDecoder(unrolls, up_convs, blocks=blocks, transfer_data=transfer_data, distance_field=True)
...
class UpConv_sigmoid(nn.Module):
    def __init__(self, in_channels, out_channels, blocks=0, unroll=0, residual=False,
                 batch_norm=False, transfer_data=True):
        super(UpConv_sigmoid, self).__init__()
        self.residual = residual
        self.bn = []
        self.unroll = None
        self.transfer_data = transfer_data
...
class MeshEncoder(nn.Module):
    def __init__(self, pools, convs, fcs=None, blocks=0, global_pool=None, nl_block=0):
        ...
        self.convs = []
        if nl_block ==102:
            nl_place = np.arange(0,len(convs))
            nl_block = 2
        elif nl_block > 50: #place both before second last and last pool
...
class MeshDecoder(nn.Module):
    def __init__(self, unrolls, convs, blocks=0, batch_norm=True, transfer_data=True, distance_field=False):
        ...
        if distance_field:
            self.final_conv = UpConv_sigmoid(convs[-2], convs[-1], blocks=1, unroll=False,
                                 batch_norm=False, transfer_data=False)
        else:
